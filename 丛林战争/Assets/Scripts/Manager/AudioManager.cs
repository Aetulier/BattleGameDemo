...
public class AudioManager : BaseManager {
    ...
    public const string Sound_Bg_Fast = "Ba(fast)";
    ...
    private AudioSource normaAudioSource;
    public override void OnInit()
    {
        GameObject audioSourceGO = new GameObject("AudioSource(GameObject)");
        bgAudioSource = audioSourceGO.AddComponent<AudioSource>();
        normaAudioSource= audioSourceGO.AddComponent<AudioSource>();
        PlaySound(bgAudioSource, LoadSound(Sound_Bg_Moderate), 0.0f, true);
    }
    ...
    {
        PlaySound(normaAudioSource, LoadSound(soundName), 0.3f);
    }
